...
class SetupBiped:
    def __init__(self, number, a, q, pos, rm, sign, offset, b = None, parent = None, qreal = None):
        self.number = number
        self.child = []
        self.sister = None
        self.a = a
        ...
        #print("Joint",self.number)
        #print("Joint parent",self.parent)
        #print("b",self.b)
    ...
    def Convert2Dynamixel(self):
        #converts angle to dynamixel rotation
        print("joint",self.number)
        print("sign",self.sign)
        print("q angle", self.q * 180/(math.pi))
        self.qreal = round(4095/360 * (self.offset + (self.sign* self.q * 180/(math.pi))))
        print("qreal before cond",self.qreal)
        if self.qreal > 4095:
            self.qreal = self.qreal - 4095
        elif self.qreal < 0:
            self.qreal = self.qreal + 4095

    def Create_qReal_Array(self, joint_array):
        positions = []
        for joint in joint_array:
            positions.append(joint.qreal)
            print("positions qreal", joint.qreal)
        print("positions from function", positions)
        return positions
    ...
    def InverseKinematics(self, target_pos, target_rm):
        ...
        #print("joint",self.number)
        #print("joint parent 1", self.parent)
        ...
        for joint in joint_array:
            joint.Convert2Dynamixel()

        positions = self.Create_qReal_Array(joint_array)
...
#           number, a, q,  pos,              rm,    sign, offset, b = None, parent = None, qreal = None):
